...
const NewsletterSentHistory: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      params.append('where[newsletter][equals]', id.toString())

      // Add search filter if query exists
      if (searchQuery.length >= 2) {
        params.append('where[or][0][subscriber.name][contains]', searchQuery)
        params.append('where[or][1][subscriber.email][contains]', searchQuery)
      }
    ...
    <div style={{ marginTop: '20px' }}>
      ...
      {isLoading ? (
        ...
        <div style={{ padding: '20px', textAlign: 'center', backgroundColor: '#f9f9f9', borderRadius: '4px' }}>
          {searchQuery ? 'No matching records found.' : 'No history found for this newsletter.'}
        </div>
      ) : (
        <>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #eee', textAlign: 'left' }}>
                <th style={{ padding: '12px 10px' }}>Subscriber</th>
              ...
              {logs.map((log) => (
                <tr key={log.id} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: '12px 10px' }}>
                    {typeof log.subscriber === 'object' ? (
                      <div>
                        <strong>{log.subscriber.name || 'No Name'}</strong>
                        <br />
                        <span style={{ fontSize: '12px', color: '#666' }}>{log.subscriber.email}</span>
                      </div>
                    ) : (
                      log.subscriber
                    )}
...
export default NewsletterSentHistory
