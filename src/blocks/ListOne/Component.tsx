          ...
          <ul className="flex flex-col" style={{ marginTop: `${marginTop}`, marginLeft: `${marginLeft}` }}>
            {lists.map((list, index) => (
              <li key={index} className="flex items-start justify-start" style={{ marginTop:`${space}` }}>
                <span className="left-number text-[darkred] text-xs mr-3" style={{ fontFamily: 'Big Shoulders Display', marginTop: '5px' }}>{String(index + 1).padStart(2, '0')}</span>
                <div className="flex flex-col">
                  { list.title && <span className="text-[15px]/6 text-gray-600 tracking-normal" style={{ fontFamily: 'Big Shoulders Display' }}>{list.title}</span> }
